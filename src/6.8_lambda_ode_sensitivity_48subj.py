# -*- coding: utf-8 -*-
"""Full-cohort sensitivity analysis for the B-W ODE-loss weight.

The only changed quantity is lambda_ode.  All architecture, preprocessing,
optimization, features, and evaluation settings match the key full-cohort
ablation implementation used in Table 2.  Conditions are run sequentially and
saved after every completed weight, so an interrupted run can be resumed.
...
Usage
-----
python 6.8_lambda_ode_sensitivity_48subj.py 42

This experiment is prespecified for seed 42 and retrains PIHD at
lambda_ode = 0.25, 0.5, 1, 2, and 4 on all 48 participants.
"""
...
LAMBDA_ODE_VALUES = [0.25, 0.5, 1.0, 2.0, 4.0]
...
def pihd_denoise_ablation(t_np, hbo_norm, device=DEVICE,
    ...
    for ep in range(N_EPOCHS_S2):
        ...
        loss=loss_data+lambda_ode*loss_eq+0.5*loss_ic+loss_f_rest+loss_hrf+loss_var
...
ablation_conditions = [
    {
        'name': f'lambda_ode={value:g}',
        'lambda_ode': float(value),
        'use_ode': True,
        'use_rest_init': True,
        'use_rest_constraint': True,
        'use_var_reg': True,
    }
    for value in LAMBDA_ODE_VALUES
]

checkpoint_path = os.path.join(NPY_DIR, f'lambda_ode_sensitivity_48subj_seed{SEED}_checkpoint.npy')
...
for cond in ablation_conditions:
    ...
    all_results[cond_name] = {
        ...
        'me_t': me_t, 'mi_t': mi_t,
        'me_p': me_p, 'mi_p': mi_p,
        'lambda_ode': float(cond['lambda_ode']),
...
# ================================================================
save_payload = {
    'analysis': 'lambda_ode sensitivity',
    'seed': SEED,
    'n_subjects': len(selected_subj),
...
save_path = os.path.join(NPY_DIR, f'lambda_ode_sensitivity_48subj_seed{SEED}.npy')
...
for name, result in all_results.items():
    json_payload['results'][name] = {
        ...
        'lambda_ode': float(result['lambda_ode']),
...
json_path = os.path.join(JSON_DIR, f'lambda_ode_sensitivity_48subj_seed{SEED}.json')
