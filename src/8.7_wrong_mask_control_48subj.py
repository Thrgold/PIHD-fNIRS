# -*- coding: utf-8 -*-
"""Wrong-task-timing control for the 48-participant PIHD experiment.

PIHD is refitted with the correct temporal prior (0 s) and four circularly
shifted priors (-8, -4, +4, +8 s). Stage-1 rest fitting, the Stage-2 resting
drive constraint, the task-variance mask, and the transient HRF prior are
shifted together. Evaluation remains fixed at the true task times.
...
Usage
-----
python 8.7_wrong_mask_control_48subj.py 42 --zero-only
python 8.7_wrong_mask_control_48subj.py 42
python 8.7_wrong_mask_control_48subj.py 123
python 8.7_wrong_mask_control_48subj.py 999
...
ZERO_ONLY = '--zero-only' in sys.argv[2:]
...
def shifted_temporal_priors(t_np, shift_seconds, period=32.0):
    """Return masks/prior, reproducing the main model exactly at zero shift.

    The previous control rebuilt the zero-shift HRF on a dense grid and then
    interpolated it. Because the HRF construction is sampling-dependent, that
    changed even the nominal control model. Here the prior is always first
    generated on the experiment's original 80-point grid. Zero shift returns
    that array without interpolation; nonzero conditions circularly shift the
    same sampled prior.
    """
    t_np = np.asarray(t_np, dtype=float)
    shift_seconds = float(shift_seconds)
    original_prior = make_hrf_prior_torch(t_np, onset=8.0, dur=8.0)

    if np.isclose(shift_seconds, 0.0, atol=1e-12):
        rest_mask = (t_np < 7.5) | ((t_np > 16.5) & (t_np < 23.5))
        task_mask = ((t_np >= 8.0) & (t_np < 16.0)) | ((t_np >= 24.0) & (t_np < 32.0))
        return rest_mask, task_mask, original_prior.copy()

    base_t = np.mod(t_np - shift_seconds, period)
    rest_mask = (base_t < 7.5) | ((base_t > 16.5) & (base_t < 23.5))
    task_mask = ((base_t >= 8.0) & (base_t < 16.0)) | ((base_t >= 24.0) & (base_t < 32.0))
    hrf_prior = np.interp(base_t, t_np, original_prior)
    return rest_mask, task_mask, hrf_prior
...
def pihd_denoise_ablation(t_np, hbo_norm, device=DEVICE,
    ...
    rest_mask_np, task_mask_np, hrf_prior = shifted_temporal_priors(
        t_np, mask_shift_seconds
    )
...
mask_conditions = [
    {'name': 'Correct mask (0 s)', 'mask_shift_seconds': 0.0},
    {'name': 'Wrong mask (-8 s)',  'mask_shift_seconds': -8.0},
    {'name': 'Wrong mask (-4 s)',  'mask_shift_seconds': -4.0},
    {'name': 'Wrong mask (+4 s)',  'mask_shift_seconds': 4.0},
    {'name': 'Wrong mask (+8 s)',  'mask_shift_seconds': 8.0},
]
conditions_to_run = mask_conditions[:1] if ZERO_ONLY else mask_conditions
if ZERO_ONLY:
    print('Validation mode: running only the exact zero-shift condition.')

checkpoint_path = os.path.join(NPY_DIR, f'wrong_mask_control_v2_48subj_seed{SEED}_checkpoint.npy')
if os.path.exists(checkpoint_path):
    checkpoint = np.load(checkpoint_path, allow_pickle=True).item()
    if checkpoint.get('control_version') != CONTROL_VERSION:
        raise ValueError(f"Incompatible checkpoint version: {checkpoint_path}")
...
for cond in conditions_to_run:
    ...
    for i, tr in enumerate(selected_trials):
        ...
        sig = pihd_denoise_ablation(
            ...
            use_ode=True,
            use_rest_init=True,
            use_rest_constraint=True,
            use_var_reg=True,
            mask_shift_seconds=cond['mask_shift_seconds']
        )
    ...
    all_results[cond_name] = {
        ...
        'me_t': me_t, 'mi_t': mi_t,
        'me_p': me_p, 'mi_p': mi_p,
        'me_p_bonferroni_10': min(float(me_p) * 10, 1.0),
        'mi_p_bonferroni_10': min(float(mi_p) * 10, 1.0),
        'mask_shift_seconds': float(cond['mask_shift_seconds']),
    ...
    np.save(checkpoint_path, {
        ...
        'conditions': [c['name'] for c in mask_conditions],
        'mask_shifts_seconds': [c['mask_shift_seconds'] for c in mask_conditions],
        'results': all_results,
    }, allow_pickle=True)
...
# ================================================================
# Save a complete NPY plus a human-readable JSON summary.
# ================================================================
save_payload = {
    'seed': SEED,
    'control_version': CONTROL_VERSION,
    ...
    'control_description': (
        'Stage-1 rest masks, Stage-2 rest/task constraints, and the HRF prior '
        'are circularly shifted together; evaluation windows remain unshifted.'
    ),
    'results': all_results,
}
save_path = os.path.join(NPY_DIR, f'wrong_mask_control_v2_48subj_seed{SEED}.npy')
...
json_payload = {
    ...
    'control_description': save_payload['control_description'],
    'mask_shifts_seconds': save_payload['mask_shifts_seconds'],
    'results': {}
}
for name, result in all_results.items():
    json_payload['results'][name] = {
        ...
        'me_p_bonferroni_10': float(result['me_p_bonferroni_10']),
        'mi_p_bonferroni_10': float(result['mi_p_bonferroni_10']),
        'mask_shift_seconds': float(result['mask_shift_seconds']),
...
json_path = os.path.join(JSON_DIR, f'wrong_mask_control_v2_48subj_seed{SEED}.json')
